  ...
  (lambda (exp)
    (cases expression exp
           ...
           [if-exp (condition true false)
                   (cons 'if (map unparse-exp (list condition true false)))]
           [let-exp (ids vals bodies) 
                    (append (list 'let (map list ids (map unparse-exp vals)))
                            (map unparse-exp bodies))]
           [named-let-exp (name ids vals bodies)
                          (append (list 'let name (map list ids (map unparse-exp vals)))
                                  (map unparse-exp bodies))]
           [let*-exp (ids vals bodies)
                     (append (list 'let* (map list ids (map unparse-exp vals)))
                             (map unparse-exp bodies))]
           [letrec-exp (ids vals bodies)
                       (append (list 'letrec (map list ids (map unparse-exp vals)))
                               (map unparse-exp bodies))])))
